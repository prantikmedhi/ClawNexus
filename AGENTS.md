# ClawNexus Office — AI Agent Development Guide

**Created and maintained by [Prantik Medhi](https://github.com/prantikmedhi)**

---

## Overview

This document provides comprehensive development context and standards for developers working on ClawNexus Office. It covers project architecture, coding standards, integration patterns, and best practices for maintaining this multi-agent visualization platform.

## Project Overview

ClawNexus Office is the professional visual monitoring and management frontend for the [OpenClaw](https://github.com/openclaw/openclaw) Multi-Agent system. It connects to the OpenClaw Gateway via WebSocket to visualize AI agent collaboration as a digital office and provides comprehensive console management interfaces.
...
Isometric-style 2D/3D virtual office for real-time AI agent status visualization:
...
**Created by [Prantik Medhi](https://github.com/prantikmedhi) | [GitHub Repository](https://github.com/prantikmedhi/openclaw-office)**

## Advanced Execution Model

- Start each task by identifying the user-facing outcome, the runtime boundary it touches, and the smallest verifiable slice.
- Prefer localized changes unless the protocol, configuration surface, or visualization model requires a cross-cutting update.
- Surface assumptions explicitly whenever the digital-office metaphor or Gateway contract could hide ambiguity.

## Quality Gates

- Preserve same-origin proxy semantics and existing auth/token handling unless the task explicitly changes them.
- Keep React state transitions deterministic across streaming, reconnect, error, and abort paths.
- Verify TypeScript strictness, i18n coverage, test impact, and operator diagnostics before calling work complete.

## Review Lens

- Look for desynchronization between runtime config injection, frontend bootstrap logic, and production server behavior.
- Watch for UI polish gaps in loading, empty, and failure states across office view and console pages.
- Treat documentation, specs, and verification notes as part of the shipped surface, not post-work cleanup.
