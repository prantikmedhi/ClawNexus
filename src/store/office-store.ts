...
const THEME_STORAGE_KEY = "clawnexus-theme";
const CHAT_DOCK_HEIGHT_KEY = "clawnexus-chat-dock-height";
...
export const useOfficeStore = create<OfficeStore>()(
  immer((set) => ({
    ...
    setCustomization: (patch: Partial<OfficeStore["customization"]>) => {
      set((state) => {
        state.customization = { ...state.customization, ...patch };
      });
    },
    ...
    setChatDockHeight: (height: number) => {
      ...
      try {
        localStorage.setItem(CHAT_DOCK_HEIGHT_KEY.replace("openclaw", "clawnexus"), String(height));
      } catch {
        // localStorage unavailable
      }
    },
    ...
    setFloorColor: (color: string) => {
      set((state) => {
        state.customization.floorColor = color;
      });
    },

    toggleBeams: () => {
      set((state) => {
        state.customization.showBeams = !state.customization.showBeams;
      });
    },
  })),
);
